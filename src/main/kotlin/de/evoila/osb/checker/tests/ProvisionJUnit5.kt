...
class ProvisionJUnit5 : TestBase() {
    ...
    @Test
    fun runInvalidFetchTest() {
        val instanceId = UUID.randomUUID().toString()
        val statusCode = provisionRequestRunner.runFetchProvisionRequestSync(instanceId)
        assertTrue("404 MUST be returned if the Service Instance does not exist but was $statusCode instead")
    ...
    fun runSyncTest(): List<DynamicNode> {
        ...
        dynamicNodes.add(
                dynamicContainer("should handle sync requests correctly", listOf(
                        ...
                        dynamicTest("Sync PUT request check response url - not null and valid") {
                            val dashboardUrl = provisionRequestRunner.runPutProvisionRequestResponseSync(instanceId, provisionRequestBody)
                            assertTrue("dashboard_url - not null")
                            {dashboardUrl.isNotEmpty()}
                            println(" GEtDASH ::: " + dashboardUrl + " :: "+provisionRequestRunner.getDashBoard(dashboardUrl))
                            assertTrue ("dashboard_url - is valid")
                            {provisionRequestRunner.getDashBoard(dashboardUrl) == 200 }
                        },

                        dynamicTest("Sync Fetch request") {
                            val statusCodeFetch = provisionRequestRunner.runFetchProvisionRequestSync(instanceId)
                            assertTrue("Should return  200 in case of a sync service broker or 404 if it's async but it was $statusCodeFetch.")
                            { statusCodeFetch in listOf(200, 404) }
                        },
